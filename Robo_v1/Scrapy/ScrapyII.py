...
#Função para varrer todas as páginas dos Editais
def varre_Editais(dadosEdital):
...
paginacao = soup.find('ul', { "class": "p-paginator-pages ng-star-inserted"})
editais = []
...
while paginacao.find('a', { 'class': 'next page-numbers'}):
    ...
    #https://doe.sea.sc.gov.br/index.php/buscar-jornal/?cp=1 mudar para uma página só
    response = urlopen(url)
    html = response.read()
    soup = BeautifulSoup(html, 'html.parser')
    paginacao = soup.find('ul', { "class": "pagination wpdm-pagination pagination-centered text-center"})
    dadosEdital = []
    varre_Editais(dadosEdital)
